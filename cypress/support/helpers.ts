...
Cypress.Commands.add('getValue', { prevSubject: true }, element => {
  switch (element.prop('tagName')) {
    ...
    case INPUT:
    case DATETIME:
      return cy.wrap(element).find('input,textarea').invoke('val');
  }
...
Cypress.Commands.add('closeNotifications', () => {
  cy.get('ngx-notification-container').iff('.ngx-notification-close', $el => $el.click());
});
...
Cypress.Commands.overwrite('clear', (originalFn, element, options) => {
  switch (element.prop('tagName')) {
    ...
    case DATETIME:
      ...
      return cy.wrap(element);
    case SELECT:
      cy.wrap(element).iff('.ngx-select-clear', $el => $el.click());
      return cy.wrap(element);
  }
...
Cypress.Commands.add('fill', { prevSubject: true }, (subject, text, options) => {
  switch (subject.prop('tagName')) {
    case SELECT:
      return cy.wrap(subject).clear().type(`${text}{downarrow}{enter}`, options);
    case INPUT:
    case DATETIME:
    case CODEMIRROR:
