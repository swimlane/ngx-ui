describe('Buttons', () => {
  ...
  describe('button', () => {
    ...
    it('does not fucus on disable buttons', () => {
      cy.get('@SUT').within(() => {
        cy.get('.ngx-section-content').realClick({ x: 10, y: 10 });  // Navigates to first section
        cy.realPress('Tab').realPress('Tab').realPress('Tab').realPress('Tab').realPress('Tab').realPress('Tab').realPress('Tab');  // Navigates to last button
        cy.realPress('Tab');
  ...
  describe('ngx-button', () => {
    ...
    it('does not fucus on disable buttons', () => {
      cy.get('@SUT').within(() => {
        cy.get('.ngx-section-content').realClick({ x: 10, y: 10 });  // Navigates to first section
        cy.realPress('Tab').realPress('Tab').realPress('Tab').realPress('Tab').realPress('Tab').realPress('Tab').realPress('Tab').realPress('Tab');  // Navigates to last button
        cy.realPress('Tab');
        cy.focused().should('have.prop', 'tagName', 'PRE')
      });
    });
