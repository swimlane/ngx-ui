...
export class SelectDropdownComponent implements AfterViewInit {
  ...
  @Output() selectDropdownKeyup = new EventEmitter<{
    event: KeyboardEvent;
    value?: string;
  }>();
  @Output() selection = new EventEmitter<SelectDropdownOption>();
  @Output() selectDropdownClose = new EventEmitter<boolean | undefined>();
  ...
  get isNotTemplate(): boolean {
    return !(typeof this.allowAdditionsText === 'object');
  }
  ...
  onInputKeyUp(event: KeyboardEvent): void {
    ...
    if (key === KeyboardKeys.Escape) {
      this.selectDropdownClose.emit(true);
    ...
    this.updateFilterQueryIsInOptions();
    this.selectDropdownKeyup.emit({ event, value });
  }

  onShiftEnterKeyDown(event: KeyboardEvent) {
    if (this.allowAdditions) {
      this.onAddClicked(event as unknown as MouseEvent, this.filterQuery || '');
    }
  }
  ...
  private calculateGroups(
    ...
    for (const option of options) {
      ...
      const group = (
        (option as { name: string; value: unknown }).value as Record<
          string,
          unknown
        >
      )[groupBy];
