  ...
  implements OnInit, OnChanges
{
  ...
  addDefaultItemForSchemaBuilder(dataType: JsonSchemaDataType): void {
    this.schema.items = dataType.schema as Record<string, unknown>;
    if (this.schemaRef) {
      this.schemaRef.items = dataType.schema as Record<string, unknown>;
    }

    if (this.schemaRef && this.schemaRef.items) {
      this.model.push(
