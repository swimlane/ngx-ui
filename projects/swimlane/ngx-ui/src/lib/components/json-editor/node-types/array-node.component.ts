  ...
  template: `
    <div [hidden]="!expanded">
      <div *ngFor="let value of model; let i = index; trackBy: arrayTrackBy">
          ...
          <ngx-dropdown [showCaret]="true">
            <ngx-dropdown-toggle>
              <div class="type-icon" tooltipPlacement="top">
                <ngx-icon [fontIcon]="dataTypeMap[schemas[i].type] ? dataTypeMap[schemas[i].type].icon : 'integration'"></ngx-icon>
              </div>
            </ngx-dropdown-toggle>
            <ngx-dropdown-menu class="ngx-dropdown-dark-outline">
              <ul class="vertical-list">
                <li>
                  <button type="button" (click)="deleteArrayItem(i)">Delete</button>
                </li>
              </ul>
            </ngx-dropdown-menu>
          </ngx-dropdown>
        ...
        <ngx-json-editor-node [model]="value"
          (modelChange)="updateArrayItem(i, $event)" [schema]="schemas[i]"
          [path]="path + '[' + i + ']'"
          [errors]="errors">
